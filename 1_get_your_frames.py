"""
Detailed Steps in PDF [MoreD.pdf]

1_get_your_frames.py, 2_get_your_face.py based on  TalkNet-ASD. You need download this project (https://github.com/TaoRuijie/TalkNet-ASD) and pip environments first.
4_face_alignment.py, 5_eye_mouth_crop.py based on  EyeLipCropper. You need download this project (https://github.com/zhliuworks/EyeLipCropper) and pip environments first. 
"""
...
# get video path
# video_path = "/data/conggaoxiang/V2C_Data/MovieAnimation"
# video_path_list = []
...
video_path = "/data/conggaoxiang/chemistry_lectures/chem_release_version/sent_level_videos"
video_path_list = []
for speaker in os.listdir(video_path):
    video_speaker_path = os.path.join(video_path, speaker)
    for file in os.listdir(video_speaker_path):
        if os.path.splitext(file)[1] == ".mp4":
            video_path_list.append(os.path.join(video_speaker_path, file))
...
def visualization(tracks, scores, args):
    ...
    vOut = cv2.VideoWriter(os.path.join(args.pyaviPath, 'video_only.avi'), cv2.VideoWriter_fourcc(*'XVID'), 25,
                           (fw, fh))
    colorDict = {0: 0, 1: 255}
    for fidx, fname in tqdm.tqdm(enumerate(flist), total=len(flist)):
        ...
        for face in faces[fidx]:
            ...
            cv2.putText(image, '%s' % (txt), (int(face['x'] - face['s']), int(face['y'] - face['s'])),
...
# Main function
def main():
    with open("/data/conggaoxiang/Chem_wav16_feature/All_list.txt", 'r') as file:
        content_list = file.read().splitlines()
    a = 0
    for full_name in tqdm(content_list):
        ...
        a = a + 1
        print("============NUM:{}---ID:{}---SUCCESS========".format(a, full_name))
        args.videoPath = os.path.join("",
                                      base_name,
                                      "{}.mp4".format(full_name[-3:]))  # change you path [input]


        save = "{}-face-{}".format(base_name, full_name)

        args.pyframesPath = os.path.join("", save)  # change you path [output]
